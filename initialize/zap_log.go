...
func InitializeZap() (logger *zap.Logger) {
	dir := global.GVA_SERVER.System.Logdir
	if dir == "" {
		dir = "log"
	}
	// 判断log文件夹是否存在
	if ok, _ := utils.PathExist(dir); !ok {
		log.Printf("创建新文件夹:%v\n", dir)
		os.Mkdir(dir, os.ModePerm)
	}
	var consoles bool
	if global.GVA_SERVER.System.Model == "test" {
		consoles = true
	} else if global.GVA_SERVER.System.Model == "formal" {
		consoles = false
	} else {
		consoles = true
	}
	fmt.Printf("global.GVA_SERVER.System.Model: %v\n", global.GVA_SERVER.System.Model)

	//new 一个zap
	z := New_zap(dir, consoles)
	core := z.GetZapCores()
